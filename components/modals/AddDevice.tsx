...
async function createDevice(_url: string, { arg }: { arg: AddDeviceForm }) {
	const { data } = await api.post("/devices/create", arg);
	return data;
}
...
const AddDevice = React.memo(({ opened, handleClose }: ModalProps) => {
	const [active, setActive] = useState(0);
	const { trigger } = useSWRMutation("/devices/create", createDevice);
	...
	const formik = useFormik<AddDeviceForm>({
		...
		onSubmit: async (values, { setSubmitting, resetForm }) => {
			...
			try {
				...
				const message =
					err?.response?.data?.error ?? err?.message ?? "Failed to add device";
				toast.error(message);
			} finally {
				setSubmitting(false);
			}
			...
			<form>
				<div className="p-8">
					<Stepper active={active} onStepClick={setActive} size="xs">
							...
							<div className="space-y-4 mt-4">
								<div className="grid grid-cols-2 gap-3">
									...
									<TextInput
										...
										error={
											formik.touched.asset_engine && formik.errors.asset_engine
										}
									/>
								</div>
							</div>
						...
						<Stepper.Step label="Device" description="Information">
							<div className="space-y-4 mt-4">
								<div className="grid grid-cols-2 gap-3">
									<Select
										label="Device Type"
						...
						<Stepper.Step label="Agent" description="Information">
							<div className="space-y-4 mt-4">
								<Select
									label="Fitting Agent (optional)"
								...
								<div className="grid grid-cols-2 gap-x-8 gap-y-4">
									...
									<div>
										...
										<div className="space-y-2">
											...
											/>
											<ReviewRow label="SIM" value={formik.values.device_sim} />
											<ReviewRow
