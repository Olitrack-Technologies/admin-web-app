...
async function fetchPage(url: string): Promise<AssetsPage> {
...
function Assets() {
	...
	const getKey = (pageIndex: number): string =>
		`/assets?page=${pageIndex + 1}&limit=${PAGE_SIZE}`;
	...
	const {
		...
		error,
		mutate
	} = useSWRInfinite<AssetsPage>(getKey, fetchPage, {
		revalidateFirstPage: false
	});

	const assets: Asset[] = pages?.flatMap((p) => p.data) ?? [];
